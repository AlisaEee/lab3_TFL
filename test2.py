...
class Grammar:
    ...
    def compute_follow(self):
        ...
        # Если задан стартовый символ, добавляем '$' в его FOLLOW
            ...
            for A, rhs_list in self.rules.items():
                for rhs in rhs_list:
                    ...
                    for i, symbol in enumerate(rhs):
                        ...
                        # Инициализация FOLLOW для символа, если он еще не существует
                        if symbol not in self.follow:
                            self.follow[symbol] = set()

                        # Сравнение длины множества FOLLOW перед обновлением
                        initial_size = len(self.follow[symbol])
                        # Здесь вы можете добавить логику для обновления self.follow[symbol]
                        #self.follow[symbol].add(symbol)
    ...
    def compute_last(self):
        ...
        while changed:
            ...
            for A, rhs_list in self.rules.items():
                for rhs in rhs_list:
                    if rhs:  # Проверяем, что rhs не пустое
                        last_symbol = rhs[-1]
    ...
    def reverse(self):
        ...
        for A, productions in self.rules.items():
            for production in productions:
                # Реверсируем правую часть и преобразуем в кортеж
    ...
    def compute_precedes(self):
        rev = self.reverse().compute_follow()  # Вычисляем FOLLOW
        return rev
    ...
    for line in input_str.split("\n"):
        ...
        if line != "":
            ...
            if len(parts) == 2:
                ...
                #print(rhs_str)
                # Находим все терминалы и нетерминалы
                # Используем регулярное выражение для поиска терминалов и нетерминалов
                #rhs = re.findall(r'[a-z]+|[A-Z][0-9]?|\[[A-Z]+[0-9]*\]', rhs_str)
                rhs = re.findall(r'[a-z]+|[A-Z][0-9]?|\[[A-Z]+[0-9]*\]', rhs_str)
                #print(rhs)
                # rhs = re.findall(r'[a-z]+|[A-Z][0-9]*|\[[A-Z][0-9]*\]', rhs_str)rhs = re.findall(r'[a-z]+|[A-Z]+|[A-Z][0-9]?[A-Z][0-9]?', rhs_str)
                #print("rhs_str",rhs)
                combined_rhs = []
                current_sequence = []
                for symbol in rhs:
                    if symbol.startswith('[') and symbol.endswith(']'):  # Нетерминал в скобках
                        if current_sequence:  # Если есть накапливаемая последовательность
                            combined_rhs.append(current_sequence)
                            #print(combined_rhs,'combined_rhs',symbol[1:-1])
                            combined_rhs=[]
                ...
                if current_sequence:  # Если осталась последовательность в конце
                    combined_rhs.append(current_sequence)
...
def eliminate_chain_rules(grammar):
    ...
    def add_rule(new_rhs, nonterm):
        ...
        for rule in curr_rules:
           # print('rule',rule,len(rule),all(symbol.isupper() for symbol in rule))
            if all(symbol.isupper() for symbol in rule) and len(rule) == 1:  # Если правило состоит только из нетерминалов
                for n in rule:
                    if n.isupper():
    ...
    start_symbol = list(grammar.rules.keys())[0]  # Предполагаем, что стартовый символ - это первый ключ
    reachable = set()
    stack = [start_symbol]
    while stack:
        current = stack.pop()
...
def matches(grammar, symbol, string, index,result):
    #print(f"Checking symbol: {symbol}, string: {string}, index: {index}")

    if index == len(string):
        return True  # Используем список для изменения значения
    
    # Перебираем все правила для данного символа
    for rule in grammar.rules[symbol]:
        current_index = index
        #print(f"Trying rule: {rule}")

        for part in rule:
            if part.islower():  # Предполагаем, что терминалы - строчные буквы
                #print(f"Terminal {part}, comparing with {string[current_index] if current_index < len(string) else 'out of bounds'}")
                if current_index < len(string) and string[current_index] == part:
                    #print('current_indexd',current_index,string[current_index],part)
                    current_index += 1
                    #print('current_index',current_index,string[current_index],part)
                else:
                    # Если терминал не совпадает, выходим из цикла, но продолжаем с другим правилом
                    break
            else:  # Если часть - нетерминал, рекурсивно пытаемся его сопоставить
                #print(f"Non-terminal {part}, asking for match")
            ...
            # Если мы прошли все части правила и не встретили break, значит правило сработало
            return True
...
def random_walk(bigram_matrix,FIRST,LAST, start_symbol):
    ...
    while bigram_matrix[current_symbol] and random.random() < 0.05:
...
def genString():
    ...
    #print("Bigram",bigram_matrix)
   #print(generate_string(bigram_matrix, 'S'))
    sequence = random_walk(bigram_matrix,FIRST,LAST, new_grammar.start_symbol)
    return sequence

def generateTests(number):
    with open('tests.txt', 'w') as f:
        for i in range(number):
            print("FILE OTPU")
            string = genString()
            f.write(string+' '+str(belongs_to_language(grammar, string))+'\n')
    f.close()
...
string4 = "ck"
# Читаем грамматику
grammar = read_grammar(input_grammar)
...
print(belongs_to_language(new_grammar, s))
'''
print(belongs_to_language(new_grammar, string1))
#print(belongs_to_language(new_grammar, string2))
#print(belongs_to_language(new_grammar, string3))
#print(belongs_to_language(new_grammar, string4))
